<% const title = 'Staff Sign in'; const active = ''; %>
<%- include('../partials/header.js', { title, active, user: null, staff: null }) %>

<div id="login-root" aria-label="Staff sign in">
  <style>
    /* Same scoped styles as student login */
    #login-root * { box-sizing: border-box; }
    #login-root {
      min-height: calc(100vh - 120px);
      display: grid; place-items: center; padding: 24px 16px; background: #f8fafc;
    }
    #login-root .card {
      width: 100%; max-width: 420px; background: #fff; border: 1px solid #e5e7eb;
      border-radius: 16px; padding: 22px; box-shadow: 0 6px 18px rgba(0,0,0,0.06);
    }
    #login-root .brand { display:flex; align-items:center; gap:10px; justify-content:center; margin-bottom:14px; }
    ...
    #login-root input[type="text"], #login-root input[type="password"] {
      width:100%; border:1px solid #e5e7eb; border-radius:10px; padding:10px 12px; background:#fff; line-height:1.35; font:inherit; outline:none;
    }
    #login-root input:focus { border-color:#7c3aed; box-shadow:0 0 0 3px rgba(124,58,237,0.12); }

    #login-root .error {
      background:#fee2e2; color:#991b1b; border:1px solid #fecaca; padding:8px 10px; border-radius:10px; margin-bottom:12px; font-size:.95rem;
    }
    ...
    #login-root .btn {
      ...
      border:1px solid #7c3aed; background:#7c3aed; color:#fff; padding:10px 14px; border-radius:12px; font-weight:600; cursor:pointer; text-decoration:none;
  ...
  <div class="card" role="form" aria-describedby="login-desc">
    ...
    <h1>Staff sign in</h1>
    <p id="login-desc" class="lead">Log in to view classes, reviews, and manage training & job adverts.</p>

    <% if (typeof error !== 'undefined' && error) { %>
      <div class="error"><%= error %></div>
    <% } %>

    <form method="post" action="/staff/login" novalidate>
      <label class="field">
      ...
      <div class="actions">
        ...
        <a class="link" href="/login">Student sign in</a>
      </div>
    </form>
  </div>
</div>
